...
class TestToJSON(unittest.TestCase):
    ...
    def test_to_json_single_player(self):
        player = Player("Alpha", "alpha@gmail.com",
                        "2000-04-04", 455, "Berserk")
        ...
        self.assertEqual(
            res, result, "To JSON: The test for a single player failed")
    ...
    def test_to_json_multiple_players(self):
        ...
        self.assertEqual(
            res, results, "From JSON: The test for a multiple players failed")
...
class TestFromJSON(unittest.TestCase):
    def test_from_json_single_player(self):
        ...
        for i in range(len(res)):
            self.assertEqual(res[i].nickname, player[i].nickname,
                             "From single JSON: Nicknames are different")
            self.assertEqual(res[i].email, player[i].email,
                             "From single JSON: Emails are different")
            self.assertEqual(res[i].date_of_birth, player[i].date_of_birth,
                             "From single JSON: Dates of birth are different")
            self.assertEqual(res[i].xp, player[i].xp,
                             "From single JSON: XP are different")
            self.assertEqual(res[i].cls, player[i].cls,
                             "From single JSON: Classes are different")
    ...
    def test_from_json_multiple_player(self):
        ...
        for i in range(len(res)):
            self.assertEqual(res[i].nickname, players[i].nickname,
                             "From multiple JSON: Nicknames are different")
            self.assertEqual(res[i].email, players[i].email,
                             "From multiple JSON: Emails are different")
            self.assertEqual(res[i].date_of_birth, players[i].date_of_birth,
                             "From multiple JSON: Dates of birth are different")
            self.assertEqual(res[i].xp, players[i].xp,
                             "From multiple JSON: XP are different")
            self.assertEqual(res[i].cls, players[i].cls,
                             "From multiple JSON: Classes are different")
...
class TestToXML(unittest.TestCase):
    def test_to_xml_single_player(self):
        ...
        '''
        players = [Player(nickname="Alpha", email="alpha@gmail.com",
                          date_of_birth="2000-04-04", xp=455, cls="Berserk")]
        factory = PlayerFactory()
...
class TestFromXML(unittest.TestCase):
    def test_from_xml_single_player(self):
        ...
        for i in range(len(gen_players)):
            self.assertEqual(
                gen_players[i].nickname, players[i].nickname, "From single XML: Nicknames are different")
            self.assertEqual(
                gen_players[i].email, players[i].email, "From single XML: Emails are different")
            self.assertEqual(gen_players[i].date_of_birth, players[i].date_of_birth,
                             "From single XML: Dates of birth are different")
            self.assertEqual(
                gen_players[i].xp, players[i].xp, "From single XML: XP are different")
            self.assertEqual(
                gen_players[i].cls, players[i].cls, "From single XML: Classes are different")
    ...
    def test_from_xml_multiple_plauers(self):
        ...
        for i in range(len(gen_players)):
            self.assertEqual(gen_players[i].nickname, players[i].nickname,
                             "From multiple XML: Nicknames are different")
            self.assertEqual(
                gen_players[i].email, players[i].email, "From multiple XML: Emails are different")
            self.assertEqual(gen_players[i].date_of_birth, players[i].date_of_birth,
                             "From multiple XML: Dates of birth are different")
            self.assertEqual(
                gen_players[i].xp, players[i].xp, "From multiple XML: XP are different")
            self.assertEqual(
                gen_players[i].cls, players[i].cls, "From multiple XML: Classes are different")
