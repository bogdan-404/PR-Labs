...
class TestToProtobuf(unittest.TestCase):
    def test_to_protobuf_single_player(self):
        ...
        self.assertEqual(
            res, result, "To Protobuf: The test for a single player failed")
    ...
    def test_to_protobuf_multiple_players(self):
        ...
        self.assertEqual(
            res, result, "To Protobuf: The test for multiple plauers failed")
...
class TestFromProtobuf(unittest.TestCase):
    def test_from_protobuf_single_player(self):
        ...
        for i in range(len(gen_players)):
            self.assertEqual(gen_players[i].nickname, player[i].nickname,
                             "From multiple XML: Nicknames are different")
            self.assertEqual(
                gen_players[i].email, player[i].email, "From multiple XML: Emails are different")
            self.assertEqual(gen_players[i].date_of_birth, player[i].date_of_birth,
                             "From multiple XML: Dates of birth are different")
            self.assertEqual(
                gen_players[i].xp, player[i].xp, "From multiple XML: XP are different")
            self.assertEqual(
                gen_players[i].cls, player[i].cls, "From multiple XML: Classes are different")
